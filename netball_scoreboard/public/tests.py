...
class PublicModelsTestCase(TestCase):
    def setUp(self):
...
class PublicViewsTestCase(TestCase):
    ...
    def test_live_match_list_view(self):
        response = self.client.get(reverse('public:live_match_list'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'public/live_match_list.html')

    def test_match_scoreboard_view(self):
        response = self.client.get(reverse('public:match_scoreboard', args=[self.match.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'public/match_scoreboard.html')
